...
export default function PaperDetailActions({ paperId, title }: Props) {
  ...
  function handleDelete() {
    startTransition(async () => {
      ...
      if (r.success) {
        toast.success(`已删除整套试卷及 ${r.deletedQuestions ?? 0} 道题`);
  ...
  return (
    <>
      <button
        ...
        className="shrink-0 flex items-center gap-1.5 px-3 py-1.5 rounded-lg border border-red-200 dark:border-red-900/60 text-xs text-red-600 dark:text-red-400 hover:bg-red-50 dark:hover:bg-red-950/30 transition-colors"
      >
        <Trash2 size={12} /> 删除整套试卷
      </button>
