...
// 沉浸式 PDF 阅读器（react-pdf / PDF.js）。
// ⚠️ 必须由父级 next/dynamic(..., { ssr:false }) 懒载 —— react-pdf 触碰 DOMMatrix/Canvas，SSR 会崩。
//
// 能力：
//   · 左侧可折叠侧栏：目录(PDF 内置书签树) / 缩略图，点击跳页（解决「只能一页页翻」）。
//   · 主阅读区 @tanstack/react-virtual 虚拟化，几百页只挂载视口内 <Page>，内存恒定。
//   · 大文件优化：disableAutoFetch（共享 PDF_OPTIONS），仅按可见页取数 KB。
...
export default function PdfViewerModal({ item, onClose }: Props) {
  const scrollRef = useRef<HTMLDivElement>(null);
  // eslint-disable-next-line @typescript-eslint/no-explicit-any
  const pdfRef = useRef<any>(null);
  const [numPages, setNumPages] = useState(0);
  const [pageWidth, setPageWidth] = useState(() =>
    typeof window !== 'undefined' ? Math.min(window.innerWidth - 280, 900) : 800,
  );
  ...
  return (
    <Drawer.Root open onOpenChange={(o) => !o && onClose()} direction="bottom">
      <Drawer.Portal>
        <Drawer.Overlay className="fixed inset-0 z-50 bg-black/60 backdrop-blur-sm" />
        <Drawer.Content
          className="fixed inset-x-0 bottom-0 z-50 flex h-[94vh] flex-col rounded-t-2xl bg-zinc-100 outline-none dark:bg-zinc-950"
          aria-describedby={undefined}
        >
          {/* 顶栏 */}
          <div className="flex shrink-0 items-center gap-2 border-b border-zinc-200 px-3 py-2.5 dark:border-zinc-800">
            <div className="absolute left-1/2 top-1.5 h-1 w-10 -translate-x-1/2 rounded-full bg-zinc-300 dark:bg-zinc-700" />
            <button
              type="button"
            ...
            <Drawer.Title className="min-w-0 flex-1 truncate text-sm font-semibold text-zinc-900 dark:text-zinc-100">
              {item.title}
            </Drawer.Title>
            ...
            {numPages > 0 && (
              <div className="flex items-center gap-1 text-xs text-zinc-600 dark:text-zinc-400">
                <button
                  ...
                  className="rounded-md p-1 hover:bg-zinc-200 disabled:opacity-40 dark:hover:bg-zinc-800"
                  aria-label="上一页"
                >
                  <ChevronUp size={16} />
                </button>
          ...
          {error ? (
            ...
            <Document
              ...
              onLoadSuccess={(pdf) => {
                ...
                pdf
                  .getOutline()
                  .then((o) => {
                    setOutline((o as OutlineNode[] | null) ?? []);
                    if (!o || o.length === 0) setSideTab('thumbs');
                  })
                  .catch(() => setOutline([]));
              }}
              onLoadError={(e) => setError('PDF 加载失败：' + e.message)}
            ...
            >
              ...
              {numPages > 0 && sidebarOpen && (
                <aside className="flex w-56 max-w-[72vw] shrink-0 flex-col border-r border-zinc-200 bg-white dark:border-zinc-800 dark:bg-zinc-900">
              ...
              >
                {numPages > 0 && pageWidth > 0 && (
                  <div className="relative mx-auto w-full" style={{ height: virtualizer.getTotalSize(), maxWidth: 900 }}>
                    {virtualItems.map((vi) => (
                      <div
                        key={vi.key}
                        data-index={vi.index}
