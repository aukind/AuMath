...
import { getFollowingOf, getMyFollowingIds } from '@/app/actions/follows';
...
export async function generateMetadata({ params }: { params: Promise<{ id: string }> }) {
  ...
  return { title: profile ? `${profile.username} 的关注 · AuMath` : '关注 · AuMath' };
}
...
export default async function UserFollowingPage({ params }: { params: Promise<{ id: string }> }) {
  ...
  const [{ data: { user } }, list, followingIds] = await Promise.all([
    ...
    <div className="min-h-screen bg-zinc-50 dark:bg-zinc-950">
      ...
      <main className="mx-auto max-w-2xl px-4 py-8">
        <h1 className="mb-5 text-xl font-bold text-zinc-900 dark:text-zinc-50">
          {profile.username} 的关注
          <span className="ml-2 text-sm font-medium text-zinc-400 tabular-nums">{list.length}</span>
        </h1>
        <UserList
          users={list}
